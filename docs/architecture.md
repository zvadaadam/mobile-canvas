...
[Native adapters](native-adapters.md) investigates extracting the current Expo implementation behind project/build and native frame boundaries, then adding SwiftUI/UIKit iOS projects. It records the DEUS app findings, source/schema migration, native rebuild and capture freshness, navigation/isolation limits and staged acceptance criteria. This is a proposal; the current implementation still supports the Expo paths described above.
