...
- One primary native macOS window in Expo's look: live frames with names above them, flow connectors between them, arrange, pan, zoom, inspector, undo and agent setup. `open --screen <key>` reveals one screen at 100%, like opening a URL, so a change can be checked without clicking through the app.
...
- Experimental Swift target support shares the same native canvas and command loop. Source-only SwiftSyntax analysis maps recognized navigation, presentations and panes independently of preview availability; existing `#Preview` factories and source-derived local demo recipes supply live UI, and missing-data destinations remain visible. Swift edits rebuild the native host, and ordinary Swift navigation is not yet automatically pinned. See `docs/architecture.md`.
- Optional embedded MCP App review: the agent client can display the screen flow and actual native captures over the same runtime. Native controls and editing remain in the native canvas; the embedded review does not create another project model or imitate iOS controls.
