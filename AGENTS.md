...
- One human surface: the native canvas window. Screen arrangement, context, fixture editing, history and agent connection all live there. CLI and MCP are agent adapters over the same loopback runtime, not additional human products. No agent chat, no hidden generation loop, no accounts, media generation or cloud storage.
...
- Runtime code lives in `src/runtime`, shared contracts in `src/shared`, the native host in `apps/native-host`, the preview hooks in `packages/preview`. Everything is TypeScript except what must not be: the `bin` entry, Metro's CommonJS config and the Expo config plugin, and the Swift host.
